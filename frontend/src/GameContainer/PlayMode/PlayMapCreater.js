...
const PlayMapCreater = (props) => {
  ...
  useEffect(() => {
    if (
      ...
      // console.log(`Time ${status.endTime - status.startTime}`);
  ...
  useEffect(() => {
    ...
    if (status.gameState !== "playing") cancelAnimationFrame(requestId);
    ...
    <>
      <div>
        <canvas
          ...
          //   style={{ position: "absolute", left: 0, top: 150, zIndex: 0 }}
        ></canvas>
      </div>
      ...
      <div className="form-popup" id="gameForm">
        ...
        <button className="gameButton" onClick={closeForm}>
          Try Again
        </button>
      </div>
