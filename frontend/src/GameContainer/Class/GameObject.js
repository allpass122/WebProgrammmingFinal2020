...
const isCollision = (obj1, obj2) => {
  ...
  if (
    (obj1.type === "sphere" && obj2.type === "cube") ||
    (obj2.type === "sphere" && obj1.type === "cube")
  ) {
    const sObj = obj1.type === "sphere" ? obj1 : obj2;
    const cObj = obj1.type === "cube" ? obj1 : obj2;
    if (
      sObj.pos.between(
        cObj.pos.sub(cObj.size.mul(0.5)).sub(new Vec2(sObj.r, 0)),
        cObj.pos.add(cObj.size.mul(0.5)).add(new Vec2(sObj.r, 0))
      )
    )
      return true;
    if (
      sObj.pos.between(
        cObj.pos.sub(cObj.size.mul(0.5)).sub(new Vec2(0, sObj.r)),
        cObj.pos.add(cObj.size.mul(0.5)).add(new Vec2(0, sObj.r))
      )
    )
      return true;
    const pointList = [
      ...
      cObj.pos.add(new Vec2(p[0] * cObj.size.x, p[1] * cObj.size.y))
    );
    return Math.min(pointList.map((p) => p.sub(sObj.pos))) <= sObj.r;
  }
  if (obj1.type === "cube" && obj2.type === "cube") {
    return (
      Math.abs(obj1.pos.x - obj2.pos.x) <= obj1.size.x / 2 + obj2.size.x / 2 &&
      Math.abs(obj1.pos.y - obj2.pos.y) <= obj1.size.y / 2 + obj2.size.y / 2
    );
  }
  return false;
};
...
export class spikedBlock {
  ...
  enpackage() {
    const originPos = this.gridPos
      .mul(w)
      .add(constant.mapStart)
      .add(new Vec2(0.5 * w, 0.5 * w));
    return {
      type: this.type,
      id: this.id,
  ...
  collision(target) {
    switch (target.type) {
      case "sphere":
        if (
          isCollision(
            { type: "cube", pos: this.pos, size: new Vec2(w, w) },
            target
          )
        )
          return "block";
        if (
          (this.detail.leftSpike &&
            isCollision(
              {
                type: "cube",
                pos: this.pos.add(new Vec2(-0.625 * w, 0)),
                size: new Vec2(0.25 * w, 0.75 * w),
              },
              target
            )) ||
          (this.detail.rightSpike &&
            isCollision(
              {
                type: "cube",
                pos: this.pos.add(new Vec2(0.625 * w, 0)),
          ...
          (this.detail.upSpike &&
            isCollision(
              {
                type: "cube",
                pos: this.pos.add(new Vec2(0, -0.625 * w)),
                size: new Vec2(0.25 * w, 0.75 * w),
              },
              target
            )) ||
  ...
  place(map, objects = null) {
    ...
    if (
      constant.typeLayerPairs[map[gridPos.y][gridPos.x].type].isOverlap(
        this.layer
      ) ||
      (this.detail.leftSpike &&
        gridPos.x - 1 >= 0 &&
        constant.typeLayerPairs[map[gridPos.y][gridPos.x - 1].type].isOverlap(
          this.layer
        )) ||
      (this.detail.upSpike &&
        gridPos.y - 1 >= 0 &&
        constant.typeLayerPairs[map[gridPos.y - 1][gridPos.x].type].isOverlap(
          this.layer
        )) ||
      (this.detail.rightSpike &&
        gridPos.x + 1 < constant.mapSize.x &&
        constant.typeLayerPairs[map[gridPos.y][gridPos.x + 1].type].isOverlap(
          this.layer
        )) ||
      (this.detail.downSpike &&
        gridPos.y + 1 < constant.mapSize.y &&
        constant.typeLayerPairs[map[gridPos.y + 1][gridPos.x].type].isOverlap(
          this.layer
        ))
    ) {
    ...
    if (
      map[gridPos.y][gridPos.x].layer.isOverlap(this.layer) ||
      (this.detail.leftSpike &&
        gridPos.x - 1 >= 0 &&
        map[gridPos.y][gridPos.x - 1].layer.isOverlap(this.layer)) ||
      (this.detail.upSpike &&
        gridPos.y - 1 >= 0 &&
        map[gridPos.y - 1][gridPos.x].layer.isOverlap(this.layer)) ||
      (this.detail.rightSpike &&
        gridPos.x + 1 < constant.mapSize.x &&
        map[gridPos.y][gridPos.x + 1].layer.isOverlap(this.layer)) ||
      (this.detail.downSpike &&
    ...
    if (this.detail.leftSpike && gridPos.x - 1 >= 0)
      map[gridPos.y][gridPos.x - 1].layer.add(this.layer);
    if (this.detail.upSpike && gridPos.y - 1 >= 0)
      map[gridPos.y - 1][gridPos.x].layer.add(this.layer);
    if (this.detail.rightSpike && gridPos.x + 1 < constant.mapSize.x)
      map[gridPos.y][gridPos.x + 1].layer.add(this.layer);
    if (this.detail.downSpike && gridPos.y + 1 < constant.mapSize.y)
      map[gridPos.y + 1][gridPos.x].layer.add(this.layer);
  ...
  remove(map, objects = null) {
    ...
    if (this.detail.leftSpike && gridPos.x - 1 >= 0)
      map[gridPos.y][gridPos.x - 1].layer.sub(this.layer);
    if (this.detail.upSpike && gridPos.y - 1 >= 0)
      map[gridPos.y - 1][gridPos.x].layer.sub(this.layer);
    if (this.detail.rightSpike && gridPos.x + 1 < constant.mapSize.x)
      map[gridPos.y][gridPos.x + 1].layer.sub(this.layer);
    if (this.detail.downSpike && gridPos.y + 1 < constant.mapSize.y)
      map[gridPos.y + 1][gridPos.x].layer.sub(this.layer);
  ...
  collision(target) {
    switch (target.type) {
      case "sphere":
        if (
          isCollision(
            { type: "cube", pos: this.pos, size: new Vec2(0.7 * w, 0.7 * w) },
            target
          )
        )
          return "platform";
...
export class movingPlatform {
  ...
  detailFunction() {
    return {
      ...
      speed: {
        type: "select",
        options: ["super slow", "slow", "normal", "fast", "super fast"],
      },
    };
  }
  enpackage() {
  ...
  update(objects = null) {
    ...
    const dirVec = Vec2.direction(this.detail.direction).mul(
      this.detail.distance * w
    );
    const originPos = this.gridPos
      .mul(w)
      .add(constant.mapStart)
    ...
    if (
      ~~(
        (Date.now() - this.lastRecord) /
        parameters[this.detail.speed] /
        this.detail.distance
      ) %
        2 ===
      0
    ) {
      this.pos = originPos.add(
        dirVec.mul(
          ((Date.now() - this.lastRecord) /
            parameters[this.detail.speed] /
            this.detail.distance) %
...
export class bow {
  ...
  draw(ctx) {
    ...
    let lifeCycle = this.active
      ? (Date.now() - this.lastFire) / parameters[this.detail.RoF]
      : 0;
...
export class arrow {
  ...
  update(objects) {
    if (
      !this.pos.between(
        constant.mapStart,
        constant.mapStart.add(constant.mapSize.mul(w))
      )
    )
      return { type: "destory" };
    for (let i = 0; i < objects.length; i++) {
      if (objects[i].id !== this.id && objects[i].collision) {
        let result = objects[i].collision({
          type: "sphere",
          pos: this.pos,
          r: 0.1,
        });
  ...
  collision(target) {
    switch (target.type) {
      case "sphere":
        if (isCollision({ type: "sphere", pos: this.pos, r: 0.1 }, target))
          return "arrow";
        return "none";
      default:
        return "none";
...
export class cymbal {
  ...
  collision(target) {
    switch (target.type) {
      case "sphere":
        if (isCollision({ type: "sphere", pos: this.pos, r: 0.48 * w }, target))
          return "cymbal";
        return "none";
      default:
        return "none";
  ...
  draw(ctx) {
    ...
    let r =
      lifeCycle < pivot
        ? 0.05 + (0.4 * lifeCycle) / pivot
        : 0.05 + (0.4 * (1 - lifeCycle)) / (1 - pivot);
...
export class cymbalWave {
  ...
  collision(target) {
    switch (target.type) {
      case "sphere":
        if (isCollision({ type: "sphere", pos: this.pos, r: this.r }, target))
          return "cymbalWave";
        return "none";
      default:
        return "none";
  ...
  draw(ctx) {
    ...
    ctx.globalAlpha =
      0.25 *
      Math.max(
        1.1 - Math.pow(this.r / (w * parameters[this.detail.range]), 16),
        0
      );
